...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    return [_excelModel.rowOfSection[section] integerValue];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (_delegate && [_delegate respondsToSelector:@selector(GWExcelTableView:viewForHeaderInSection:)]) {
        [_delegate GWExcelTableView:tableView viewForHeaderInSection:section];
    }
    return nil;
}
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    if (_delegate && [_delegate respondsToSelector:@selector(GWExcelTableView:heightForHeaderInSection:)]) {
        [_delegate GWExcelTableView:tableView heightForHeaderInSection:section];
    }
    return 0.0001f;
}
